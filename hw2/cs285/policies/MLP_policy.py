...
class MLPPolicy(BasePolicy):
    ...
    def build_baseline_forward_pass(self):
        self.baseline_prediction = tf.squeeze( \
            build_mlp(self.observations_pl, output_size=1, scope='nn_baseline', n_layers=self.n_layers, size=self.size))
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        if self.nn_baseline:
            ...
            self.baseline_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.baseline_loss)
    ...
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        ...
        _, loss = self.sess.run(
            ...
            feed_dict={self.observations_pl: observations, self.actions_pl: acs_na, self.adv_n: adv_n})
        ...
        if self.nn_baseline:
            ...
            self.sess.run( \
                [self.baseline_op, self.baseline_loss], 
                feed_dict={self.targets_n: targets_n}) 
                ## NOTE: should I feed self.observation_pl here as well since it's feeded once above??
        return loss
